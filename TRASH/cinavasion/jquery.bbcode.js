...
(function($){
  $.fn.bbcode = function(options){
	...
	if(options.tag_bold){
      text = text + '<a href="#" id="strong" title="">';
      /*
	...
	if(options.tag_paragr){
      text = text + '<a href="#" id="p" title="">';
      /*
	  if(options.button_image){
        text = text + '<img src="' + options.image_url + 'bold.png" />';
      }else{
	...
	if(options.tag_break){
      text = text + '<a href="#" id="br" title="">';
     /*
	 if(options.button_image){
        text = text + '<img src="' + options.image_url + 'bold.png" />';
      }else{
	...
	$('#bbcode_bb_bar a').click2(function() {
      var button_id = $(this).attr("id");
      var start = '['+button_id+']';
     var end = '</'+button_id+'>';
...
else if (e.which == 73 && isCtrl == true) // CTRL + I, make - LI
{ 
	$("#li").click();
	return false;
} 
else if (e.which == 85 && isCtrl == true) // CTRL + U, make - UL
{ 
	$("#ul").click();
	return false;
}
else if (e.which == 80 && isCtrl == true) // CTRL + P, paragraph
{ 
	$("#p").click();
	return false;
}
else if (e.which == 81 && isCtrl == true) // CTRL + P, paragraph
{ 
	$("#br").click2();
	return false;
}
